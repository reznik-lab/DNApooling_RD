...
cohort_select = "lung" ; {
    if(tolower(cohort_select) == "renal"){
        maf = read.csv("ST1_MAF_TRACERx_RCC.csv",stringsAsFactors = F) ; cohort = "RCC"
        #Filter data: in the RCC cohort bilateral and multifocal tumors are excluded
        blmf_tumors_exclude = c("K097","K114","K334","K352","K265")
            blmf_tumors = paste(blmf_tumors_exclude, collapse="|")
            maf = maf[-grep(blmf_tumors,maf$patient_ID),]
    }
    if(tolower(cohort_select) == "lung"){
        maf = read.csv("ST2_MAF_TRACERx_NSCLC.csv",stringsAsFactors = F) ; cohort = "NSCLC"
        maf$mutation_ID = gsub("\\.","-",maf$mutation_ID)}
...
#Clonality definitions: CCF to define a variant as 'clonal' (>= threshold)
...
max_region_num = 10 # Number of regions to include in the analysis (all possibilities from 1:max_region_num will be included)
spec_region_num = 3 # Specifies the specific simulation where more detailed results will be shown (must be less than max_region_num)
    ...
    for(region_num in 1:max_region_num){
        ...
        for(t in 1:n_tumors){
                ...
                all_muts = sapply(regions_all, function(x) x$mutation_ID)
                    ...
                    res_eventlvl[eventids,"mutation_ID"] = all_muts
                    res_eventlvl[eventids,"tumor_ID"] = tumorid
                    res_eventlvl[eventids,"tumor_ID_raw"] = tumor
                    res_eventlvl[eventids,"Gene"] = sapply(strsplit(all_muts,split="_"), function(x)x[1])
                    res_eventlvl[eventids,"regions"] = paste(region_select,collapse=";")
                    ...
                    oncokb = maf[mafindex,c("mutation_ID","OncoKB_evidencelvl")]
                        ...
                        res_eventlvl[eventids,"OncoKB_evidencelvl"] = evidence_targets[all_muts]
                ...
                detection_events_mr = sapply(all_muts,function(event){
                    ...
                    return(detect)})
                    res_eventlvl[eventids,"mr_detection"] = detection_events_mr
                ...
                detection_events_pool = sapply(all_muts, function(event){
                    ...
                    res_eventlvl[eventids,"pool_detection"] = detection_events_pool
                ...
                clonality_truth = sapply(all_muts, function(event){
                    ...
                    res_eventlvl[eventids,"true_clonality"] = clonality_truth
                ...
                clonality_mr = sapply(all_muts, function(event){
                      ccf = sapply(regions, function(x){
                        ...
                        return(res)})
                      clonality = callClonal_MR(ccf, cutpoint = MR_threshold)})   
                    res_eventlvl[eventids,"mr_clonality"] = clonality_mr
                ...
                clonality_pool = sapply(all_muts, function(event){
                    ...
                    res_eventlvl[eventids,"pool_ccf"] = ccf_pool
                    res_eventlvl[eventids,"pool_clonality"] = clonality_pool  
                    res_eventlvl[eventids,"pool_pur"] = pur_pool
                ...
                if(cohort == "RCC"){    
                    ...
                    subtype_pool = subtypes_txRCC_POOL(multiregion_list = regions, clonal_threshold = pool_ccf_threshold)
                        res_eventlvl[eventids,"mr_subtype"] = subtype_mr
                        res_eventlvl[eventids,"pool_subtype"] = subtype_pool}
        }                    
        ...
        dropout_eventlvl_mr =  1 - (sum(res_eventlvl$mr_detection)/nrow(res_eventlvl))
            dropout_eventlvl_pool = 1 - (sum(res_eventlvl$pool_detection)/nrow(res_eventlvl))
            
            #Proportion of patients with at least 1 mutation missed
            dropout_patientlvl_mr =  1 - (sum(res_patientlvl$mr_detection)/nrow(res_patientlvl))
            dropout_patientlvl_pool = 1 - (sum(res_patientlvl$pool_detection)/nrow(res_patientlvl))
        ...
        detected_mr = which(res_eventlvl$mr_detection == 1) ; detected_pool = which(res_eventlvl$pool_detection == 1)
        clonalityerror_eventlvl_mr = ifelse(res_eventlvl$mr_clonality[detected_mr] == res_eventlvl$true_clonality[detected_mr],1,0)
            clonalityerror_eventlvl_mr = 1 - (sum(clonalityerror_eventlvl_mr)/length(detected_mr)) 
            clonalityerror_eventlvl_pool = ifelse(res_eventlvl$pool_clonality[detected_pool] == res_eventlvl$true_clonality[detected_pool],1,0)
            clonalityerror_eventlvl_pool = 1 - (sum(clonalityerror_eventlvl_pool)/length(detected_pool)) 
            
            #Proportion of patients with at least 1 mutation misclassified in terms of clonality
            clonalityerror_patientlvl_mr = 1 - (sum(res_patientlvl$mr_clonality)/nrow(res_patientlvl)) 
            clonalityerror_patientlvl_pool = 1 - (sum(res_patientlvl$pool_clonality)/nrow(res_patientlvl)) 
        ...
        targets_mr = sum(res_patientlvl$mr_targets)
        targets_pool = sum(res_patientlvl$pool_targets)
        ...
        #COST
        cost_sampleprep = costs$cost_persample_prep*(1+region_num) # sample prepping cost is the same for MR and POOL approaches
        cost_mr =  costs$cost_overhead + cost_sampleprep + costs$cost_seq*(1+region_num) # 1 normal sample + number of tumor samples
        cost_pool = costs$cost_overhead + cost_sampleprep + costs$cost_seq*2 # 1 normal sample + 1 pool
        
        #Cost-effectiveness measures (CE = outcome/cost)
        cea_detection_mr = (1-dropout_eventlvl_mr)/cost_mr
        cea_detection_pool = (1-dropout_eventlvl_pool)/cost_pool
        cea_clonality_mr = (1-clonalityerror_eventlvl_mr)/cost_mr
        cea_clonality_pool = (1-clonalityerror_eventlvl_pool)/cost_pool
        
        #SAVE ALL OUTCOMES AS A NAMED VECTOR
        outcomes = c('dropout_eventlvl_mr'=dropout_eventlvl_mr,'dropout_patientlvl_mr' = dropout_patientlvl_mr,
                      'dropout_eventlvl_pool'=dropout_eventlvl_pool,'dropout_patientlvl_pool' = dropout_patientlvl_pool,
                        
                      'clonalityerror_eventlvl_mr'=clonalityerror_eventlvl_mr,'clonalityerror_patientlvl_mr' = clonalityerror_patientlvl_mr,
                      'clonalityerror_eventlvl_pool'=clonalityerror_eventlvl_pool,'clonalityerror_patientlvl_pool' = clonalityerror_patientlvl_pool,
                        
                      'cost_mr' = cost_mr, 'cost_pool' = cost_pool,
                      'cea_detection_mr' = cea_detection_mr, 'cea_detection_pool' = cea_detection_pool,
                      'cea_clonality_mr' = cea_clonality_mr, 'cea_clonality_pool' = cea_clonality_pool,
                     
                      'targets_mr' = targets_mr, 'targets_pool' = targets_pool,
                      'tmbunder_mr' = tmbunder_mr, 'tmbunder_pool' = tmbunder_pool)
        
        # ADDITIONAL RCC-SPECIFIC OUTCOMES
        if(cohort == "RCC"){
            # RISK ERROR
            rccriskerror_mr = 1 - sum(res_patientlvl$mr_riskrcc)/nrow(res_patientlvl)
            rccriskerror_pool = 1 - sum(res_patientlvl$pool_riskrcc)/nrow(res_patientlvl)
            
            #CORRECT SUBTYPE ASSIGNMENT
            rccsubtype_mr = sum(res_patientlvl$mr_subtypercc)/nrow(res_patientlvl)
            rccsubtype_pool = sum(res_patientlvl$pool_subtypercc)/nrow(res_patientlvl)
            
            outcomes = c(outcomes,
                            'rccriskerror_mr' = rccriskerror_mr, 'rccriskerror_pool' = rccriskerror_pool,
                            'rccsubtype_mr' = rccsubtype_mr, 'rccsubtype_pool' = rccsubtype_pool)
        }
        ...
        #Confusion matrix of clonality calls (needs to be a numerical matrix)
        conf_matrix = as.data.frame(table(interaction(events_sampled$pool_clonality, events_sampled$true_clonality)),stringsAsFactors = F)
            confusion = matrix(0,nrow=2,ncol = 2, dimnames = list('pool' = c('clonal','subclonal'), 
                                                                  'truth' = c('clonal','subclonal')))
            #Fill the empty matrix
            for(cat in 1:nrow(conf_matrix)){
              val = conf_matrix$Freq[cat]
              pool = strsplit(conf_matrix$Var1[cat],split = "\\.")[[1]][1]
              truth = strsplit(conf_matrix$Var1[cat],split = "\\.")[[1]][2]
              confusion[pool,truth] = val
            }
            tp = confusion['clonal','clonal'] ; fp = confusion['clonal','subclonal']
            tn = confusion['subclonal','subclonal'] ; fn = confusion['subclonal','clonal']
...
clean_outcome_names = c("Dropout rate (event-level)","Dropout rate (patient-level)",
                        "Clonality error rate (event-level)","Clonality error rate (patient-level)",
...
clean_classifier_names = c("AUC of the ROC curve",
                           ...
                           "False-negative rate (miss-rate)", "False-positive rate (fallout)","Informedness (Youden's J statistic)",
                           "Positive-predictive value (precision)","Negative-predictive value",
                           "False-discovery rate (FDR)","False-omission rate",
                           "Markedness (deltaP)","Critical success index (threat score)",
                           "Accuracy","Balanced accuracy","F1 score","Cohen's Kappa statistic",
      ...
      if(o %in% relative_outcomes){
        relaplot = ggplot() + 
            ...
            geom_bar(data = means,aes(x = region_num, y = value_rela, group = variable, fill = variable), stat = "identity", 
                     position = "dodge", color = "black", width = 0.75) +
            ggtitle(names(outcome_measures)[outcome_measures == o],
                    subtitle = "(results shown relative to single region profiling)") +
...
classifiers_path = paste0(output_path,"plots_classifiers/")
    ...
    for(cl in names(classifiers)){
        ...
        plot = ggplot(classifier_data, aes(x=regions_pooled, y=classifier_toplot, group = regions_pooled)) +
            geom_boxplot(width = 0.8, outlier.alpha = 0) +
            geom_jitter(size = 2,alpha = 0.6, color = "#015980", width = 0.15) +
...
details_path = paste0(output_path,"plots_detailed_pool",spec_region_num,"regions/")
    ...
    #Obtain the event-level results over all the iterations with the specified number of regions (defaults to 4)
    detail_res = grep(paste0("r",spec_region_num),names(details))
    ...
    targets_dropped = which(detail_allevents$pool_detection == 0 & grepl(evidence_levels,detail_allevents$OncoKB_evidencelvl)) ; {  
      ...
      pieplot_targets = plot_pie(targets_dropped$Gene, colpal = c("#55C0D5","#E9972A"),plot_title = "Targetable mutation dropout")
    ...
    if(cohort == "RCC"){
        ...
        riskcond_missed = which(detail_alltumors$pool_riskrcc == 0)
            ...
            pieplot_riskcond = plot_pie(riskcond_missed$variable, colpal = c("#00A969","#38499D","#474747"),plot_title = "Risk misclassification features")
    ...
    detail_allevents = detail_allevents[which(detail_allevents$pool_detection == 1),] ; {
        ...
        otherplots = list()
          ...
          allcutoffs = seq(0.52,0.99,0.001)
              ...
              clonality_calls = ifelse(detail_allevents$pool_ccf >= ccf_threshold,"clonal","subclonal") #Makes new clonality calls to 
                  classifier_res = as.data.frame(table(interaction(clonality_calls,detail_allevents$true_clonality)),stringsAsFactors = F)
                  confusionmat = matrix(0,nrow=2,ncol = 2, dimnames = list('pool' = c('clonal','subclonal'), 
                                                                    'truth' = c('clonal','subclonal')))
                  for(cat in 1:nrow(classifier_res)){
                    val = classifier_res$Freq[cat]
                    pool = strsplit(classifier_res$Var1[cat],split = "\\.")[[1]][1]
                    truth = strsplit(classifier_res$Var1[cat],split = "\\.")[[1]][2]
                    confusionmat[pool,truth] = val
                  }
            
                  tp = confusionmat['clonal','clonal'] ; fp = confusionmat['clonal','subclonal']
                  tn = confusionmat['subclonal','subclonal'] ; fn = confusionmat['subclonal','clonal']
            
              #Sensitivity and PPV
              sensitivity = tp/(tp+fn) # proportion of clonal events correctly classified
              ppv_precison = tp/(tp+fp) # proportion of real clonal events out of all clonal calls
    
              #Accuracy measures
              accuracy = (tp+tn)/sum(confusionmat)
              
              f1 = 2 * (ppv_precison * sensitivity)/
                       (ppv_precison + sensitivity)
              
            
              mcc =          ( (tp*tn) - (fp*fn) )/
                    sqrt((tp+fp) * (tp+fn) * (tn+fp) * (tn+fn))
              
              res = c(accuracy,f1,mcc) ; names(res) = c("Accuracy","F1 score","MCC")
              return(res)
          })
          ...
          otherplots$plot_classifiers = ggplot(accuracy_res, aes(x=ccf_threshold, y=value, group = variable, color = variable)) +
              ...
              scale_x_continuous(limits = c(0.49,1.01), breaks = seq(0.5,1,.1)) +
              scale_color_manual("Measure",values = color_palette[c(1,2,4)]) +
              xlab(paste("CCF in pool of",spec_region_num,"regions")) + ylab("Value") +
              themefull
          
          #Combining all plots and printing
          plots_combined_classifier = plot_grid(plotlist=otherplots, nrow = 1, ncol = 3, labels = c("A","B","C"))
              plotname = paste0(details_path,"classifiers_ccfpool.pdf")
              pdf(plotname,16,6)
              print(plots_combined_classifier)
              dev.off()
        }
